...
<div>
    <div class="card">
        <div class="card-body p-3 p-lg-4">
            <form method="POST" action="{{ route('combos.update', $combo) }}" enctype="multipart/form-data" novalidate>
                @csrf
                @method('PUT')
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="img-dropzone" id="imgDropzone" tabindex="0" role="button" aria-label="Subir imagen">
                                ...
                                <div class="dropzone-empty {{ $combo->image ? 'd-none' : '' }}" id="dropzoneEmpty">
                                    <i class="bi bi-image" aria-hidden="true"></i>
                                    <p>Arrastra una imagen o <span class="link">haz clic para subir</span></p>
                                    <small>JPG, PNG o WebP · máx. 2 MB</small>
                                </div>
                                <div class="dropzone-preview {{ $combo->image ? '' : 'd-none' }}" id="dropzonePreview">
                                    <img id="previewImg" src="{{ $combo->image ? asset('storage/'.$combo->image) : '' }}" alt="Vista previa">
                                    ...
                                    <table class="table table-sm align-middle mb-0" id="comboProductsTable">
                                        ...
                                        <tbody id="comboProductsBody">
                                            ...
                                            <tr data-id="{{ $product->id }}">
                                                ...
                                                <td>
                                                    <span class="text-muted">{{ $product->category->name ?? 'Sin categoría' }}</span>
                                                </td>
                                                <td>
                                                    <input type="number"
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-outline-danger btn-sm remove-product" data-id="{{ $product->id }}" aria-label="Quitar">
                                                        <i class="bi bi-x"></i>
                                                    </button>
                                                </td>
                                            </tr>
                            ...
                            <div class="card-body">
                                        ...
                                        <div class="col-6">
                                            <div class="input-group">
                                                ...
                                                <input type="number"
                                                       ...
                                                       step="0.01"
                                                       placeholder="0.00"
                                                       value="{{ old('sale_price', number_format($combo->sale_price, 2, '.', '')) }}"
                                                       required>
                                                @error('sale_price')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                    ...
                                    <div class="d-flex align-items-center gap-2 mb-1">
                                        <div class="form-check form-switch mb-0">
                                            <input class="form-check-input"
                                                   ...
                                                   role="switch"
                                                   id="roundBsToggle"
                                                   {{ old('round_bs', $combo->round_bs ?? null) ? 'checked' : '' }}>
                                        ...
                                        <select class="form-select form-select-sm" id="roundBsStep" style="width:auto;"
                                                {{ old('round_bs', $combo->round_bs ?? null) ? '' : 'disabled' }}>
                                            @foreach([5, 10, 15, 20, 25, 50, 100] as $step)
                                                <option value="{{ $step }}" {{ (int) old('round_bs', $combo->round_bs ?? 5) === $step ? 'selected' : '' }}>a {{ $step }}</option>
                                            @endforeach
                                        </select>
                                        <input type="hidden" name="round_bs" id="roundBsHidden" value="{{ old('round_bs', $combo->round_bs ?? '') }}">
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="isActive" name="is_active">
                                        <option value="1" {{ old('is_active', $combo->is_active) == '1' || old('is_active', $combo->is_active) == '1' ? 'selected' : '' }}>Activo</option>
                                        <option value="0" {{ old('is_active', $combo->is_active) == '0' || old('is_active', $combo->is_active) == '0' ? 'selected' : '' }}>Inactivo</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                ...
                <div class="mt-4 d-flex gap-2">
                    <button type="submit" class="btn btn-brand">
                        <i class="bi bi-check2-circle me-1"></i>Actualizar Combo
                    </button>
